...
import { Users, ShieldAlert, Clock, Mail, Download } from 'lucide-react';
import { collection, query, getDocs, orderBy, limit } from 'firebase/firestore';
import { db } from '../firebase';
import { handleFirestoreError, OperationType } from '../utils/firestoreErrorHandler';

export default function AdminTab({ itemVariants }: { itemVariants: any }) {
        ...
        <motion.div variants={itemVariants} className="bg-black/40 border border-white/10 rounded-[2rem] p-8 backdrop-blur-2xl">
          <div className="flex items-center justify-between mb-6">
            ...
            <button 
              ...
              className="p-2 bg-white/5 hover:bg-white/10 text-zinc-300 rounded-lg transition-colors disabled:opacity-50"
              title="Download CSV"
            >
          ...
          <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
            {loading ? (
              ...
              users.map(user => (
                <div key={user.id} className="flex items-center gap-4 p-4 rounded-2xl bg-white/5 border border-white/5">
                  <img 
          ...
          <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
            {loading ? (
              ...
              loginAttempts.map(attempt => (
                <div key={attempt.id} className="flex flex-col gap-2 p-4 rounded-2xl bg-white/5 border border-white/5">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-xs px-2 py-1 rounded-md font-bold uppercase ${
                      ...
                      attempt.status === 'attempted' ? 'bg-yellow-500/20 text-yellow-400' : 
                  ...
                  <div className="flex items-center justify-between text-xs text-zinc-500">
                    ...
                    <span className="uppercase font-bold">{attempt.type}</span>
                  </div>
                  {attempt.error && (
